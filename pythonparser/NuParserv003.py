...
def getAddress20byteP2SH(hex_string, tx_type):#this is for pay-to-script-hash 20byte
    ...
    if tx_type == "53":
        ...
        step0 = "0x40" + hex_string #40 hex = 64Dec. = S
        
    else:
        
        step0 = "0x1A" + hex_string #20 hex = 26Dec. = B
...
for x in BLOCKPPC:
    ...
    num_motions = len(x["blockVotes"]["motionHashVote"])
    ...
    """
    if num_motions > 0:#----------------------THERE ARE MOTION VOTES--------------------------------------------
        for mo in xrange(0,num_motions):
            motion_hash = x["blockVotes"]["motionHashVote"][mo]
            if motion_hash !="0000000000000000000000000000000000000000":
                moto_vote_search = MCL.find({"_id":motion_hash})
                mo_county = moto_vote_search.count()
                if mo_county==0:
                    MCL.insert({"_id":motion_hash,"cust_num":mo_count,"mo_details":{"passed":"false","blockHeight":0},"timeStamp":x["blockTimeStamp"],"block":x["blockHeight"],"num_votes":0,"total_votes":0,"latest_block":x["blockHeight"],"CDD":x["blockCDD"]})
                    mo_count+=1
    """
